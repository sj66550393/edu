	...
	private function ajax()
	{
		...
		switch($this->ev->url(4))
		{
			...
			break;
                        ...
                        case "getsubjectbygradeid":
                            $gradeid = $this->ev->get('gradeid');
                            echo "gradeid = ".$gradeid;
                            $subjectList = $this->section->getSubjectListByArgs(array(array("AND","gradeid = :gradeid" , 'gradeid',$gradeid)));
                            $data = array(array(0,'选择科目'));
			foreach($subjectList as $subject)
			{
				$data[] = array($subject['subjectid'],$subject['subject']);
			}
                        foreach($data as $p)
			{
				echo "<option value=\"{$p[0]}\">{$p[1]}</option>";
