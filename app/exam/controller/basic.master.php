...
class action extends app {
    ...
    private function selectmember() {
        ...
        if ($this->ev->get('submit')) {
            ...
            if ($search['useremail'])
                $args[] = array('AND', "useremail  LIKE :useremail", 'useremail', '%' . $search['useremail'] . '%');
            if ($search['groupid'])
    ...
    private function delsection() {
        $sectionid = $this->ev->get('sectionid');
        $knows = $this->section()->getKnowsByArgs(array(array("AND", "knowssectionid = :knowssetionid", 'knowssectionid', $sectionid)));
        if ($knows) {
            $message = array(
                'statusCode' => 300,
    ...
    private function point() {
        ...
        if (!$section['sectionid']) {
            ...
            $knowstype = $this->basic->getKnowstypeBySubjectid($section['sectionsubjectid']);
            $this->tpl->assign('section', $section);
            $this->tpl->assign('knows', $knows);
            $this->tpl->assign('subjects', $subjects);
            $this->tpl->assign('knowstype', $knowstype);
    ...
    private function addpoint() {
        if ($this->ev->get('insertknows')) {
            ...
            $knowstypeid = $args['knowstypeid'];
            foreach ($knows as $know) {
                if ($know) {
                    $data = $this->section->getKnowsByArgs(array(array("AND", "knowsstatus = 1"), array("AND", "knows = :knows", 'knows', $know), array("AND", "knowssectionid = :knowssectionid", 'knowssectionid', $args['knowssectionid']), array("AND", "knowstypeid = :knowstypeid", 'knowstypeid', $knowstypeid)));
                    if ($data) {
                        $errmsg .= $know . ',';
                    } else
                        $this->section->addKnows(array("knowssectionid" => $args['knowssectionid'], "knows" => $know, 'knowstypeid' => $knowstypeid));
                }
            }
            $errmsg = trim($errmsg, ' ,');
            if ($errmsg)
            ...
            if (!$section['sectionid']) {
                ...
                $num = count($knowstype);
                echo "num = " . $num;
                foreach ($knowstype as $value) {
                    echo $value['knowstype'] . "aaa";
                }
                $knows = $this->section->getKnowsList($page, 10, array(array("AND", "knowssectionid = :sectionid", 'sectionid', $sectionid), array("AND", "knowsstatus = 1")));
                $this->tpl->assign('section', $section);
                $this->tpl->assign('subjects', $subjects);
    ...
    private function addsubject() {
        if ($this->ev->get('insertsubject')) {
            $args = array('subject' => $this->ev->get('subject'), 'subjectsetting' => $this->ev->get('setting'), 'gradeid' => $this->ev->get('gradeid'));
            $data = $this->basic->getSubjectByNameAndGradeid($args['subject'], $args['gradeid']);
            if ($data) {
                $message = array(
                    'statusCode' => 300,
    ...
    private function savepaper() {
        ...
        foreach ($sessionvars['examsessionquestion']['questions'] as $key => $tmp) {
            if (!$questype[$key]['questsort']) {
                foreach ($tmp as $p) {
                    $questLog = array();
                    $questLog['question'] = $p['question'];
                    $questLog['questionanswer'] = $p['questionanswer'];
                    $questLog['questiondescribe'] = $p['questiondescribe'];
                    $questLog['questionselect'] = $p['questionselect'];
                    $questLog['questionselectnumber'] = $p['questionselectnumber'];
                    $questLog['useranswer'] = $nanswer;
                    $questLog['questionknowsid'] = $p['questionknowsid'];
                    $questLog['questionlevel'] = $p['questionlevel'];
                    $questLog['userid'] = $sessionvars['examsessionuserid'];
                    if (is_array($sessionvars['examsessionuseranswer'][$p['questionid']])) {
                        ...
                        if ($nanswer == $p['questionanswer']) {
                            ...
                            $questLog['score'] = 100;
                            $this->exam->addQuestLog($questLog);
    ...
    private function index() {
        ...
        if (!$search)
            ...
            $args = array();
        if ($search['basicid'])
            $args[] = array("AND", "basicid = :basicid", 'basicid', $search['basicid']);
        else {
            if ($search['keyword'])
                $args[] = array("AND", "basic LIKE :basic", 'basic', "%{$search['keyword']}%");
            if ($search['basicareaid'])
                $args[] = array("AND", "basicareaid = :basicareaid", 'basicareaid', $search['basicareaid']);
            if ($search['basicsubjectid'])
                $args[] = array("AND", "basicsubjectid = :basicsubjectid", 'basicsubjectid', $search['basicsubjectid']);
            if ($search['basicapi'])
                $args[] = array("AND", "basicapi = :basicapi", 'basicapi', $search['basicapi']);
            if ($search['basicclosed']) {
                if ($search['basicclosed'] == 1)
                    $basicclosed = 1;
                else
                    $basicclosed = 0;
