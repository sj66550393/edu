...
class basic_exam
{
	...
	//参数：无
	//返回值：年级列表数组
	public function getGradeList($args = 1)
	{
		$data = array(false,'grade',$args);
		$sql = $this->pdosql->makeSelect($data);
		return $this->db->fetchAll($sql,'gradeid','gradesetting');
	}   
        
        //根据参数获取知识点类型
	//参数：无
	//返回值：知识点类型列表
	public function getKnowstypeListByArgs($args)
	{
		$data = array(false,'knowstype',$args);
		$sql = $this->pdosql->makeSelect($data);
		return $this->db->fetchAll($sql,'knowstype');
	}  
	...
	//参数：科目id
	//返回值：知识点类型
	public function getKnowstypeBySubjectid($subjectid)
	{
		$data = array(false,'knowstype',array(array("AND","subjectid = ".$subjectid)));
		$sql = $this->pdosql->makeSelect($data);
		return $this->db->fetchAll($sql , 'knowstypeid');
	}
	...
	//参数：科目名称和年级ID字符串
	//返回值：科目信息数组
	public function getSubjectByNameAndGradeid($subject , $gradeid)
	{
		$data = array(false,'subject',array(array("AND","subject = :subject",'subject',$subject) , array("AND" , "gradeid = :gradeid" , 'gradeid' , $gradeid)));
		$sql = $this->pdosql->makeSelect($data);
		return $this->db->fetch($sql,'subjectsetting');
	}
